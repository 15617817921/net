...
import com.zhouyou.http.demo.model.Login;
...
public class LoginActivity extends AppCompatActivity {
    ...
    public void onLogin(final String name, final String pass) {
        ...
        EasyHttp.post(ComParamContact.Login.LOGIN)
...
//                .params(ComParamContact.Login.PASSWORD, MD5.encrypt4login(pass, AppConstant.APP_SECRET))
                ...
                .sign(true)
                .timeStamp(true)
                .execute(new ProgressDialogCallBack<String>(DialogUtils.loading(this)) {//ApiResult  格式如果统一 则直接Login;自己定义可String
                    @Override
                    ...
                    @Override
                    public void onSuccess(String authModel) {
                        Login login = new Gson().fromJson(authModel, Login.class);
                        String token_id = login.getToken();
                        SaveUtils.put(LoginActivity.this,Constant.Save.TOKEN_ID, token_id);
                        String[] split = token_id.split("\\|");
//                        CacheUtils.get(context).put(Constant.Save.USERNAME, login.getInfo().getName());
//                        CacheUtils.get(context).put(Constant.Save.TOKEN_ID, token_id);
//                        CacheUtils.get(context).put(Constant.Save.HEAD, login.getInfo().getHead());
//                        CacheUtils.get(context).put(Constant.Save.TOKEN, split[0]);
                        SaveUtils.put(LoginActivity.this,Constant.Save.USERID, split[1]);
                        LogUtils.E(authModel.toString());
                        showToast("登录成功！");
                        //授权类信息存入缓存
