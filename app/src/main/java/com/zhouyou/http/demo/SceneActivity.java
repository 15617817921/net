...
import com.zhouyou.http.demo.mytest.HomeTask;
import com.zhouyou.http.demo.mytest.Notice;
...
//    }  
    ...
    public void onZipRequest(View view) {
        //使用zip操作符合并等待多个网络请求完成后，再刷新界面
        //例如下面：数据来自3个不同的接口
        String authorization = (String) SaveUtils.get(SceneActivity.this, Constant.Save.TOKEN_ID, "");
        LogUtils.E("例如下面--"+authorization);
        Observable<String> mobileObservable = EasyHttp.get("/notice/personal/")
                ...
                .params("page", 1 + "")
                .params("size", 1 + "")
                .execute(new CallClazzProxy<TestApiResult1<String>, String>(String.class) {
                });

        Observable<String> searchObservable = EasyHttp.get("/mission/index/")
                .headers("Authorization",authorization)
                .params("page", 1 + "")
                .params("size", 10 + "")
                //采用代理
        ...
        //new Function3最后一个参数这里用的是List<Object>，表示将3个返回的结果，放在同一个集合最终一次性返回，你也可以指定返回其它你需要的数据类型并不一定是List<Object>
        //假如这三个接口返回的都是TestBean,那么就可以直接用具体的List<TestBean>,不需要用List<Object>
        Observable.zip(mobileObservable, searchObservable, new BiFunction<String, String, List<Object>>() {
            @Override
            public List<Object> apply(String notice, String content) throws Exception {
                LogUtils.E(notice.toString()+"--"+content.toString());//将接收到的3个数据先暂存起来，一次性发给订阅者
                List list = new ArrayList();
                list.add(notice);
                list.add(content);
                return list;
            }
        }).subscribe(new ProgressSubscriber<List<Object>>(SceneActivity.this ,mProgressDialog) {
            @Override
